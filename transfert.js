...
// ══════════════════════════════════════════════════════════════
// Permet de transférer la propriété d'un personnage, d'une
// chronique, d'un document ou d'une campagne à un autre joueur.
//
// Prérequis : avoir exécuté 10_transfer.sql dans Supabase.
// ══════════════════════════════════════════════════════════════
...
// Messages d'erreur renvoyés par la RPC Supabase
...
document.addEventListener('click', e => {
  ...
  if (modal && modal.style.display === 'flex' && !panel.contains(e.target)) {
    closeTransferModal();
  }
});
...
function _renderTransferModal() {
  ...
  const currentType = types.find(t => t.value === _transferSelectedType);
  ...
  document.getElementById('transfer-modal-panel').innerHTML = `
    ...
    <div class="transfer-field-wrap">
      ...
      <div id="transfer-user-preview" class="transfer-user-preview" style="display:none"></div>
    </div>
...
function selectTransferType(type) {
  ...
  // Réinitialise les champs et rerender
  _renderTransferModal();
  // Remet le focus sur le champ code
  requestAnimationFrame(() => {
    document.getElementById('transfer-code-input')?.focus();
  });
}

// Debounce léger pour la recherche de l'item par code
...
// Cherche l'item dans les stores en mémoire (ne fait pas de requête réseau)
...
async function confirmTransfer() {
  ...
  try {
    ...
    // ── Succès : on retire l'objet des stores locaux ──────────
    _removeFromLocalStores(_transferSelectedType, shareCode);

    closeTransferModal();
    showToast(t('transfer_success'));

    // Rafraîchit la vue courante si nécessaire
    renderList();
...
// Retire l'objet transféré des stores JS en mémoire
